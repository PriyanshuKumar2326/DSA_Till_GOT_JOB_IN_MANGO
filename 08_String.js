...
function balancedS(string){
  let temp=0;
  let count=0;
  for(let i=0;i<string.length;i++){
    {string[i]=="R" ? temp++ :temp--}
    if(temp==0) count++
  }
  return count;
} 

//Largest Common Prifix 
//Time:=O(s);
//space:=O(1)

function commonPrix(strs){
  let x=0;
...
function secondGA(strs){
  ...
  for(let i=0;i<strs.length;i++){
    ...
    let key="";
    for(let k=0;k<26;k++){
      key += String.fromCharCode(97 + k) + freqArr[k];
    }
